...
#######################导入数据######################
#导入数据
sce=readRDS('E:/mymodel/data/mesc/sce_mesc.rds')
uncorr_data=sce@assays@data@listData[["counts"]]
batch <- sce@colData$batch
celltype <- sce@colData$celltype

scaeqn_data=read.csv('E:/mymodel/methodcompare/datasets/mesc/mesc_scaeqn_output.csv',row.names = 1)
########################seurat方法#####################
library(Seurat)
library(dplyr)
library(Matrix)
...
###########################热图###########################
png('E:/mymodel/methodcompare/results/mesc/DEG_uncor.png',width = 2200, height = 1500,res=300)
p1 <- Heatmap(mat,                          #数据矩阵
...
print(p1)
dev.off()
png('E:/mymodel/methodcompare/results/mesc/DEG_scAEQN.png',width = 2200, height = 1500,res=300)
