...
#######################导入数据######################
#导入数据
sce=readRDS('E:/mymodel/data/panc 2 tech/sce_panc2tech.rds')
uncorr_data=sce@assays@data@listData[["counts"]]
batch <- sce@colData$batch
celltype <- sce@colData$celltype

scaeqn_data=read.csv('E:/mymodel/methodcompare/datasets/panc2tech/panc2tech_scaeqn_output.csv',row.names = 1)
########################seurat方法#####################
library(Seurat)
library(dplyr)
library(Matrix)
...
###########################热图###########################
png('E:/mymodel/methodcompare/results/panc 2 tech/DEG_uncor.png',width = 2200, height = 1500,res=300)
p1 <- Heatmap(mat,                          #数据矩阵
...
print(p1)
dev.off()
png('E:/mymodel/methodcompare/results/panc 2 tech/DEG_scAEQN.png',width = 2200, height = 1500,res=300)
