...
#######################导入数据######################
#导入数据
humanpanc=readRDS('E:/mymodel/data/brain/sce_brain.rds')
uncorr_data=humanpanc@assays@data@listData[["counts"]]
batch <- humanpanc@colData$batch
celltype <- humanpanc@colData$celltype

scaeqn_data=read.csv('E:/mymodel/methodcompare/datasets/brain/brain_scaeqn_output.csv',row.names = 1)
########################seurat方法#####################
library(Seurat)
library(dplyr)
library(Matrix)
...
pbmc.markers <- FindAllMarkers(object = data,logfc.threshold=0.25)
...
DEs_uncorr <- pbmc.markers$gene[pbmc.markers$p_val_adj < 1e-2 & pbmc.markers$avg_log2FC > 1]
#657

####scAEQN
data_scaeqn <- CreateSeuratObject(counts = scaeqn_data)
Idents(data_scaeqn) <- celltype
pbmc.markers_scaeqn <- FindAllMarkers(object = data_scaeqn,logfc.threshold=0.25)
##取每种细胞类型按log2FC值从大到小排序的前五个差异基因，用来绘制热图
...
DEs_scaeqn <- list()
#DEuncorr[] <- rownames(pbmc.markers[pbmc.markers$p_val_adj < 1e-5 & pbmc.markers$avg_log2FC > 2,])
DEs_scaeqn <- pbmc.markers_scaeqn$gene[pbmc.markers_scaeqn$p_val_adj < 1e-2 & pbmc.markers_scaeqn$avg_log2FC > 1]

###########################热图###########################
png('E:/mymodel/methodcompare/results/brain/DEG_uncor.png',width = 2200, height = 1500,res=300)
p1 <- Heatmap(mat,                          #数据矩阵
...
png('E:/mymodel/methodcompare/results/brain/DEG_scAEQN.png',width = 2200, height = 1500,res=300)
